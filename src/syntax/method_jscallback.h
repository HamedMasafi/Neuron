...
#   define METHOD_IMPL_P_QML_CALLBACK_VOID(class, ret, name, count, sep, ...) \
        ...
        {\
            ...
            if (__call_id) { \
                NEURON_WRAP_NAMESPACE(RemoteCallBase) *call = new NEURON_WRAP_NAMESPACE(RemoteCallBase)(callbackFunction, hub()->qmlEngine(), hub()->jsEngine());   \
...
#   define METHOD_IMPL_P_QML_CALLBACK_NONVOID(class, ret, name, count, sep, ...) \
        ...
        {  \
            qlonglong __call_id = invokeOnPeer(#name "Slot" sep __PARAMNAME(count, __VA_ARGS__));  \
            if (__call_id) { \
                NEURON_WRAP_NAMESPACE(RemoteCall)<ret> *call = new NEURON_WRAP_NAMESPACE(RemoteCall)<ret>(callbackFunction, hub()->qmlEngine(), hub()->jsEngine());   \
                addCall(__call_id, call);  \
            }   \
        }
