...
#   define METHOD_IMPL_P_META_METHOD_VOID(class, ret, name, count, sep, ...) \
    ...
    {   \
        ...
        if (__call_id) { \
            NEURON_WRAP_NAMESPACE(NeuronRemoteCallBase) *call = new NEURON_WRAP_NAMESPACE(NeuronRemoteCallBase)(const_cast<QObject *>(obj), const_cast<QMetaMethod *>(callbackMethod)); \
...
#   define METHOD_IMPL_P_META_METHOD_NONVOID(class, ret, name, count, sep, ...) \
    ...
    {   \
        qlonglong __call_id = invokeOnPeer(#name "Slot" sep __PARAMNAME(count, __VA_ARGS__));  \
        if (__call_id) { \
            NEURON_WRAP_NAMESPACE(NeuronRemoteCall)<ret> *call = new NEURON_WRAP_NAMESPACE(NeuronRemoteCall)<ret>(const_cast<QObject *>(obj), const_cast<QMetaMethod *>(callbackMethod)); \
            addCall(__call_id, call);  \
        }   \
    }
